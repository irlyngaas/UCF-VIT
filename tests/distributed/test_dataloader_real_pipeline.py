...
Two configs, chosen to cover meaningfully different branches:
...
Both configs already have parallelism settings (data_par_size=8,
tensor_par_size=1) that exactly match this file's real 8-rank srun launch,
so -- unlike Tier 3's smoke test -- no parallelism overrides are needed, only
data narrowing (reusing tests/integration/run_training_smoke.py's
compute_narrow_dict_idx, the same real-file-count-aware narrowing Tier 3
uses, rather than a third reimplementation).

Builds the data module directly via UCF_VIT.parse.parse_config +
...
calls training_scripts/train.py itself makes for dataloader.type ==
"iterative_dataloader" -- skipping only the model/optimizer/training-loop
parts this file has no need for.
"""
