...
def sample_images(model, var, device, res, precision_dt, patch_size, epoch=0, num_samples=10, twoD=False, save_path='figures', num_time_steps=1000):
    ...
    if not twoD:
        ...
        z = torch.randn(num_samples, 1, res[0], res[1])
    ...
    with torch.no_grad():
        for t in reversed(range(1, num_time_steps)):
            if not twoD:
                ...
                e = torch.randn(num_samples, 1, res[0], res[1])
    ...
    if save_path is not None:
        if not twoD:
            plot_3D_array_slices(images, filename=os.path.join(save_path, '3D_gen_%s_%i_%i_rank%i.png' %(var, epoch, res[0], dist.get_rank())))
        else:
            fig,ax = plt.subplots(10,10,figsize=(10,10),facecolor='w')
            for i in range(images[0].shape[0]):
                for j in range(len(times)):
                    ax[i,j].axis('off')
                    ax[i,j].imshow(images[j][i,:,:,0], interpolation='none', cmap='gray')
            plt.subplots_adjust(left=0.01, right=0.99, top=0.99, bottom=0.01, hspace=0.025, wspace=0.025)
            fig.savefig(os.path.join(save_path,'2D_gen_%s_%i_%i_rank%i.png' %(var, epoch, res[0], dist.get_rank())), format='png', dpi=300)
            plt.close(fig)
