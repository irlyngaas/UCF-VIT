...
class DensityQuadtree(FixedQuadTree):
    """A `FixedQuadTree` variant that scores nodes by edge density (intensity per unit area) rather than raw intensity sum.

    Weights the root/left-top node's split score by `mass * density**2` (where
    `density = mass / area`) instead of the raw edge-intensity sum used by
    `FixedQuadTree`, biasing subdivision toward small, densely-edged regions.
    """

    def __init__(self, domain, fixed_length=128, build_from_info=False, meta_info=None):
        """See `FixedQuadTree.__init__`.
        ...
        Args:
            ...
            build_from_info: If True, reconstruct `self.nodes` from `meta_info`
                instead of building the tree from scratch.
            meta_info: Node bounding-box list, used only when `build_from_info` is
                True.
        """
        super().__init__(domain, fixed_length, build_from_info, meta_info)

    def _build_tree(self):
        """Iteratively splits the highest density-weighted-score node into 4 quadrants until `fixed_length` nodes exist.

        Populates `self.nodes` as a list of `[Rect, score]` pairs. Stops early if
        the highest-scoring node's combined side length has shrunk below 4.

        Note:
            Only the root and the "left-top" (`lt`) child's score are weighted by
            `mass * density**2`; the other three children (`rt`, `lb`, `rb`) use the
            raw edge-intensity sum, same as `FixedQuadTree`.
        """
        h,w = self.domain.shape
        assert h>0 and w >0, "Wrong img size."
        root = Rect(0,w,0,h)
