...
def test_step_adaptive(data, seq, seq_ps, variables, net: UNETR, in_chans, patch_size, sqrt_len, twoD):
    if twoD:
...
##############################################################################################################
    ...
    for i in range(1):
        ...
        if adaptive_patching:
            ...
            for j in range(data_test.shape[0]):
                for k in range(data_test.shape[-1]):
                #seq_image, seq_size, seq_pos, qdt = patchify(np.moveaxis(data_test[j],0,-1))
                    seq_image, seq_size, seq_pos, qdt = patchify(np.moveaxis(data_test[j,:,:,:,k],0,-1))
                    data_test_seq.append(seq_image)
                    qdt_list.append(qdt)
                    seq_size_list.append(seq_size)
                    seq_pos_list.append(seq_pos)
            data_test_seq = torch.stack([torch.from_numpy(data_test_seq[i]) for i in range(data_test.shape[-1])]).to(device)
            seq_size = torch.stack([torch.from_numpy(seq_size_list[i]) for i in range(data_test.shape[-1])]).to(device)
            seq_pos = torch.stack([torch.from_numpy(seq_pos_list[i]) for i in range(data_test.shape[-1])]).to(device)
            ...
            for k in range(data_test.shape[-1]):
                output[idx_start:idx_end, :, :, k] = test_step_adaptive(data_test[idx_start:idx_end,:,:,:,k], data_test_seq[k], seq_ps[k], dict_in_variables[key], model, max_channels, patch_size, sqrt_len, twoD).detach().cpu().numpy()
            ...
            if tensor_par_size > 1:
                ...
                data_test = data_test.to(device)
                for k in range(data_test.shape[-1]):
                    output[idx_start:idx_end, :, :, :, k] = test_step(data_test[idx_start:idx_end,:,:,:,k], dict_in_variables[key], model).detach().cpu().numpy()
    ...
    plt.title('Pred')
    plt.savefig(f'{inference_path}/pred.png', bbox_inches='tight', dpi=200)
    plt.imshow(label[0,0,:,:,0])
    plt.colorbar(label='Value Scale')
    plt.title('GT')
    plt.savefig(f'{inference_path}/gt.png', bbox_inches='tight', dpi=200)
