...
def training_step(data, label, variables, net: UNETR, patch_size, twoD):
    ...
    # calculate the pressure gradient loss
    dimensions = (-3, -2, -1)
    dp = torch.gradient(label, dim=dimensions)
    dpred = torch.gradient(output, dim=dimensions)
    grad_losses = []
    for i in range(len(dp)):
        grad_losses.append(criterion(dpred[i], dp[i]))
    grad_loss = sum(grad_losses)
    ...
    for epoch in range(epoch_start,max_epochs):
        ...
        while counter < iterations_per_epoch:
            if tensor_par_size > 1:
                ...
                #Avoid unnecesary broadcasts if not using tensor parallelism
