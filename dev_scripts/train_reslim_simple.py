...
from UCF_VIT.simple.arch import ResSlimVIT, MAE
...
def training_step(data, label, variables, net: ResSlimVIT, patch_size, twoD):
...
##############################################################################################################
    ...
    superres_mag = conf['model']['net']['init_args']['superres_mag']
    decoder_depth = conf['model']['net']['init_args']['decoder_depth']
    cnn_ratio = conf['model']['net']['init_args']['cnn_ratio']

    dataset = conf['data']['dataset']
    assert dataset in ["neutron_upsample"], "This training script only supports neutron_upsample dataset"
    ...
    mae_decoder_depth = conf['model']['net']['init_args']['mae_decoder_depth']
    ...
    if dataset in [ "neutron_upsample"]:
        tile_size_z = None
    else:
    ...
    if adaptive_patching:
        ...
        if dataset != "imagenet" and dataset != "neutron_upsample":
...
##############################################################################################################
    ...
    scheduler = configure_scheduler(optimizer,warmup_steps,max_epochs,warmup_start_lr,eta_min)
