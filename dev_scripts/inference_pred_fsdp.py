...
##############################################################################################################
    ...
    assert dataset in ["solar"], "This training script only supports solar dataset"
...
##############################################################################################################
    ...
    #add unsharded DDP
        ...
        model = FSDP(model, device_id=local_rank, process_group= simple_ddp_group, sync_module_states=True, sharding_strategy=dist.fsdp.ShardingStrategy.NO_SHARD, mixed_precision=bfloatPolicy, forward_prefetch=True, limit_all_gathers = False )
