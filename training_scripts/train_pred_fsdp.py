...
##############################################################################################################
    ...
    assert dataset in ["turb"], "This training script only supports turb dataset"
    ...
    if dataset == "turb":
        for i,k in enumerate(nx_skip):
            assert (tile_size_x%nx_skip[k])==0, "tile_size_x must be divisible by nx_skip"
            assert (tile_size_y%ny_skip[k])==0, "tile_size_y must be divisible by ny_skip"
            assert (tile_size_z%nz_skip[k])==0, "tile_size_z must be divisible by nz_skip"
